...
(() => {
  ...
  env.scriptManager.supportsTooltips =
    env.scriptManager.name == "Tampermonkey" &&
  ...
  const settings = {
    controls: {
      // displays top-to-bottom in toolbar menu
      disableShorts: {
        type: "toggle",
        ...
        .forEach(
          (key) =>
            (config[key] = processKey(
              key,
              GM_getValue(`${app.configKeyPrefix}_${key}`, undefined),
            )),
        );
      function processKey(key, val) {
        const ctrl = settings.controls?.[key];
        if (
          val != undefined && // validate stored val
          ((ctrl?.type == "toggle" && typeof val != "boolean") ||
            (ctrl?.type == "slider" && isNaN(parseFloat(val))))
        )
          val = undefined;
        return (
          val ?? ctrl?.defaultVal ?? (ctrl?.type == "slider" ? 100 : false)
        );
      }
    },
    ...
    typeIsEnabled(key) {
      // for menu labels + notifs to return ON/OFF
      ...
      return reInvertSuffixes.test(key) && // flag in control key name
        ...
        ? !config[key]
        : config[key]; // so invert since flag reps opposite type state, else don't
    },
  };
  ...
  const domSelectors = {
    ...
    ai: {
      ...
      summary:
        "div#header[class*=expandable-metadata]:has(path[d*=M480-80q0-83])",
    },
    playables: {
  ...
  function cjsNotify(...args) {
    ...
    notificationDiv.quadrant =
      (notificationDiv.isTop ? "top" : "bottom") +
    ...
    if (
      !notifStyle ||
      parseInt(notifStyle.getAttribute("last-updated"), 10) < thisUpdated
    ) {
      ...
      notifStyle.textContent = // update prev/new style contents
        ...
        `transform: translateX(${
          ...
          !notificationDiv.isRight ? "-" : ""
        }35px) ;` +
        (shadow
          ? `--shadow: -8px 13px 25px 0 ${/\b(?:shadow|on)\b/i.test(shadow) ? "gray" : shadow};
    ...
    notificationDiv.style.top = notificationDiv.isTop
      ? vpYoffset.toString() + "px"
      : "";
    notificationDiv.style.bottom = !notificationDiv.isTop
      ? vpYoffset.toString() + "px"
      : "";
    notificationDiv.style.right = notificationDiv.isRight
      ? vpXoffset.toString() + "px"
      : "";
    notificationDiv.style.left = !notificationDiv.isRight
      ? vpXoffset.toString() + "px"
      : "";
      ...
      try {
        ...
        for (const divId of thisQuadrantQueue.slice(0, -1)) {
          ...
          const oldDiv = document.getElementById(divId),
            ...
            vOffset =
              +parseInt(oldDiv.style[offsetProp]) +
              5 +
              oldDiv.getBoundingClientRect().height;
          oldDiv.style[offsetProp] = `${vOffset}px`; // change prop
        }
      } catch (err) {
        console.warn("Failed to re-position notification:", err);
      }
    }
  ...
  function extractSelectors(obj) {
    return Object.values(obj).flatMap((val) =>
      typeof val == "object" ? extractSelectors(val) : val,
    );
  }
  ...
  const toolbarMenu = {
    state: {
      symbols: ["❌", "✔️"],
      separator: env.scriptManager.name == "Tampermonkey" ? " — " : ": ",
      words: ["OFF", "ON"],
    },
    ...
    register() {
      this.entryIDs = Object.keys(settings.controls).map((key) => {
        const ctrl = settings.controls[key];
        const menuLabel = `${
          ctrl.symbol || this.state.symbols[+settings.typeIsEnabled(key)]
        } ${ctrl.label} ${
          ctrl.type == "toggle"
            ? this.state.separator +
              this.state.words[+settings.typeIsEnabled(key)]
            : ctrl.type == "slider"
              ? ": " + config[key] + ctrl.labelSuffix || ""
              : ctrl.status
                ? ` — ${ctrl.status}`
                : ""
        }`;
        return GM_registerMenuCommand(
          ...
          () => {
            ...
            notify(
              `${ctrl.label}: ${this.state.words[+settings.typeIsEnabled(key)]}`,
            );
          },
          env.scriptManager.supportsTooltips
            ? { title: ctrl.helptip || " " }
  ...
  function notify(msg, pos = "", notifDuration = "", shadow = "") {
    if (
      ...
      !msg.includes(settings.controls.notifDisabled.label)
    )
      return;

    // Strip state word to append colored one later
    const foundState = toolbarMenu.state.words.find((word) =>
      msg.includes(word),
    );
    ...
    cjsNotify(
      ...
      pos,
      notifDuration,
      shadow || env.ui.scheme == "dark" ? "" : "shadow",
    );
    ...
    if (foundState) {
      ...
      styledStateSpan.append(foundState);
      notif.append(styledStateSpan);
    }
  }

  function syncConfigToUI(options) {
    if (options?.updatedKey == "disableShorts") {
      if (config.disableShorts && !checkShortsToRedir.id) checkShortsToRedir();
      else if (!config.disableShorts && checkShortsToRedir.id) {
        cancelAnimationFrame(checkShortsToRedir.id);
        checkShortsToRedir.id = null;
      }
  ...
  class YTP {
    ...
    static setExp(name, value) {
      if (!("EXPERIMENT_FLAGS" in this._config))
        this._config.EXPERIMENT_FLAGS = {};
    ...
    static setPlyrFlags(flags) {
      ...
      if (!("WEB_PLAYER_CONTEXT_CONFIGS" in this._config))
        this._config.WEB_PLAYER_CONTEXT_CONFIGS = {};
      for (var cfg in conCfgs) {
        var dflags = this.decodePlyrFlags(
          conCfgs[cfg].serializedExperimentFlags,
        );
  ...
  unsafeWindow.addEventListener("yt-page-data-updated", function tmp() {
    ...
    classicLogo.style.marginLeft = "5px";
    classicLogo.height = 65;
    classicLogo.src =
  ...
  getLoadedElem("#items.ytd-guide-section-renderer").then(() => {
    restoreTrending();
  });
  getLoadedElem("#items.ytd-mini-guide-section-renderer").then(() => {
    restoreTrending();
  });
  ...
  addEventListener("yt-page-data-updated", function () {
    ...
    var lds = $(
      "ytd-video-primary-info-renderer div#top-level-buttons-computed",
    );
    var like = $(
      "ytd-video-primary-info-renderer div#segmented-like-button > ytd-toggle-button-renderer",
    );
    var share = $(
      "ytd-video-primary-info-renderer div#top-level-buttons-computed > ytd-segmented-like-dislike-button-renderer + ytd-button-renderer",
    );
    ...
    like
      .querySelector("a")
      .insertBefore(
        like.querySelector("yt-formatted-string"),
        like.querySelector("tp-yt-paper-tooltip"),
      );
    try {
    ...
    like
      .querySelector("a")
      .insertBefore(
        paper.querySelector("yt-icon"),
        like.querySelector("yt-formatted-string"),
      );
    paper.outerHTML = paper.outerHTML
      .replace("<tp-yt-paper-button ", "<yt-icon-button ")
      .replace("</tp-yt-paper-button>", "</yt-icon-button>");
    paper = like.querySelector("yt-icon-button");
    paper
      .querySelector("button#button")
      .appendChild(like.querySelector("yt-icon"));
    var dislike = $(
      "ytd-video-primary-info-renderer div#segmented-dislike-button > ytd-toggle-button-renderer",
    );
    lds.insertBefore(dislike, share);
    $(
      "ytd-video-primary-info-renderer ytd-segmented-like-dislike-button-renderer",
    ).remove();
    ...
    if (dislike.getAttribute("class").includes("style-default-active")) {
      dlabel.setAttribute(
        ...
        dlabel
          .getAttribute("class")
          .replace("style-default", "style-default-active"),
      );
    }
    dislike
      .querySelector("a")
      .insertBefore(dlabel, dislike.querySelector("tp-yt-paper-tooltip"));
  ...
  async function formatCommentThread(thread) {
    if (thread.comment.commentRenderer) {
      thread.comment.commentRenderer = formatComment(
        thread.comment.commentRenderer,
      );
    }
    var replies;
    try {
      ...
      if (replies.viewRepliesIcon) {
        replies.viewReplies.buttonRenderer.icon =
          replies.viewRepliesIcon.buttonRenderer.icon;
        delete replies.viewRepliesIcon;
      }
      if (replies.hideRepliesIcon) {
        replies.hideReplies.buttonRenderer.icon =
          replies.hideRepliesIcon.buttonRenderer.icon;
        delete replies.hideRepliesIcon;
      }
      var creatorName;
      try {
        creatorName =
          replies.viewRepliesCreatorThumbnail.accessibility.accessibilityData
            .label;
        ...
        runs: [
          {
            text:
              replyCount > 1
                ? getString(viewMultiString, hl, replyCount, creatorName)
        ...
        runs: [
          {
            text:
              replyCount > 1
                ? getString("hideMulti", hl)
                : getString("hideSingular", hl),
          },
        ],
      };
    } catch (err) {}
  ...
  document.addEventListener("yt-page-data-updated", async () => {
    commentObserver.observe(document.querySelector("ytd-app"), {
      childList: true,
      subtree: true,
    });
  });
  ...
  function checkShortsToRedir() {
    if (location.pathname.startsWith("/shorts/"))
      return location.replace(
        `https://www.youtube.com/watch?v=${location.pathname.split("/")[2]}`,
      );
  ...
  const homeObserver = new MutationObserver(() => {
    if (location.pathname != locationPath) {
      // nav'd to diff page, re-observe
      locationPath = location.pathname;
      homeObserver.disconnect();
      getLoadedElem("html").then(() =>
        homeObserver.observe(document.documentElement, obsConfig),
      );
    } else if (locationPath == "/") {
      // remove regenerating homepage stuff
      if (config.adBlock)
        // remove ads
        document
          .querySelector("ytd-ad-slot-renderer")
          ?.closest("[rendered-from-rich-grid]")
          ?.remove();
      if (config.shortsBlock || config.playablesBlock) {
        // remove shelves
        document
          .querySelector(
  ...
  document.head.append(
    (window.configStyle ??= document.createElement("style")),
  );
  window.configStyle.textContent = Object.entries(domSelectors)
    .map(([key, selectors]) =>
      !config[`${key}Block`]
